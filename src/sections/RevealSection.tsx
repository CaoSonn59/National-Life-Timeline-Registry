      ...
      <CampaignLayout centered>
        <div className="w-full max-w-3xl mx-auto space-y-16 py-12">
          ...
          >
            ...
            {/* Timeline display */}
            <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4 font-mono text-sm uppercase text-registry-red">
              {copy.timeline.map((item, idx) => (
                <React.Fragment key={idx}>
                  <div className="px-4 py-2 border border-registry-red/30 bg-registry-red/5">
                    {item}
                  </div>
                  {idx < copy.timeline.length - 1 && (
                    <div className="hidden md:block w-4 h-[1px] bg-registry-red/50" />
                  )}
                  {idx < copy.timeline.length - 1 && (
                    <div className="block md:hidden h-4 w-[1px] bg-registry-red/50" />
                  )}
                </React.Fragment>
              ))}
            </div>
