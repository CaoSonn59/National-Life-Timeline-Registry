  ...
  return (
    <header className={`w-full py-6 border-b border-registry-border mb-12 transition-opacity duration-300 ${isGlitching ? 'opacity-50' : 'opacity-100'}`}>
      <div className="max-w-3xl mx-auto px-6 flex items-center gap-4">
        {/* Fake Government Logo (Geometric) */}
        <div className={`w-12 h-12 bg-registry-navy flex items-center justify-center ${isGlitching ? 'animate-glitch bg-registry-red' : ''}`}>
          <div className="w-6 h-6 border-2 border-white rounded-sm rotate-45"></div>
        </div>
        
        <div>
          <h1 className={`text-xl font-bold tracking-tight uppercase ${isGlitching ? 'animate-glitch text-registry-red' : 'text-registry-navy'}`}>
